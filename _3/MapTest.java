 ...
 */
    ...
    @Test // map to List<Integers> (age)
    void mapPersonAge(){
        List<Integer> age = PEOPLE
                .stream().map(p -> p.getAge()).toList();

        assertEquals(20, age.get(0));
        assertEquals(15,age.get(8));
        assertEquals(20,age.size());
    }

    @Test //map to List<String> (lastName)
    void mapPersonLastname(){
        List<String> lastname = PEOPLE
                .stream().map(p -> p.getLastName()).toList();

        assertEquals("Gunn", lastname.get(0));
    }

    @Test //map to List<Person.GENDER>
    void mapPersonGender(){
        List<Person.Gender> gender = PEOPLE
                .stream().map(p -> p.getGender()).toList();

        assertEquals(Person.Gender.MALE,gender.get(0));
    }
}
